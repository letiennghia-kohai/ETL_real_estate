...
logger = logging.getLogger('DothiTransformer')

class DothiTransformer:
    """
    ...
    def transform(self, raw_data_path: str) -> Dict[str, pd.DataFrame]:
        ...
        for listing in raw_data:
            ...
            url = listing.get('url') or str(uuid.uuid4())
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Error processing details for listing {listing_id}: {str(e)}")
                    self.invalid_records.append({
                        'id': listing_id,
                        'url': url,
                        'reason': f'Error processing details: {str(e)}'
                    })
                    continue
                ...
                # Process contact info with improved handling of masked data
                    ...
                    logger.error(f"Error processing contact info for listing {listing_id}: {str(e)}")
                    self.invalid_records.append({
                        'id': listing_id,
                        'url': url,
                        'reason': f'Error processing contact info: {str(e)}'
                    })
                    continue
                ...
                # Process structured address
                    ...
                    logger.error(f"Error processing address for listing {listing_id}: {str(e)}")
                    self.invalid_records.append({
                        'id': listing_id,
                        'url': url,
                        'reason': f'Error processing address: {str(e)}'
                    })
                    continue
                try:
                # Extract additional features from description
                    ...
                    logger.error(f"Error extracting features for listing {listing_id}: {str(e)}")
                    self.invalid_records.append({
                        'id': listing_id,
                        'url': url,
                        'reason': f'Error extracting features: {str(e)}'
                    })
                    continue
                ...
                # except Exception as e:
                #     logger.error(f"Error processing amenities for listing {listing_id}: {str(e)}")
                #     self.invalid_records.append({
                #         'id': listing_id,
                #         'url': url,
                #         'reason': f'Error processing amenities: {str(e)}'
                #     })
                #     continue
                listing_tracking.append({
                    'scraping_id': self.scraping_id,
    ...
    def _is_valid_listing(self, listing_data: Dict[str, Any]) -> bool:
        ...
        # Check for price or area outliers
        price_value = listing_data.get('price_value') or listing_data.get('price')
        area_value = listing_data.get('area_value') or listing_data.get('area')
        
        # if price_value is not '':
        #     if price_value < self.price_min_threshold or price_value > self.price_max_threshold:
        #         logger.warning(f"Price outlier detected: {price_value} {listing_data.get('price_unit')} for listing {listing_data.get('listing_id')}")
                # Don't invalidate - just log the warning
        
        # if area_value is not '':
        #     if area_value < self.area_min_threshold or area_value > self.area_max_threshold:
        #         logger.warning(f"Area outlier detected: {area_value} m² for listing {listing_data.get('listing_id')}")
                # Don't invalidate - just log the warning
        
        # Require at least one of price or area to be valid
        if price_value is '' and area_value is '':
            return False
    ...
    def _transform_property_listing(self, listing: Dict[str, Any], bds_id: str = "") -> Dict[str, Any]:
        ...
        # Process dates
        scraped_at = self._format_date(listing.get('scraped_at', datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        post_date = self._format_date(listing.get('post_date') or listing.get('post_date', ''))
        expiration_date = self._format_date(listing.get('expiry_date') or listing.get('expiry_date', ''))
    ...
    def _transform_raw_property_details(self, listing: Dict[str, Any], listing_id: str) -> Dict[str, Any]:
        ...
        # Extract details
        details = listing.get('details') or listing # Fallback to listing if details not found
        description = self._clean_text(listing.get('description', ''))
        
        # Extract bedroom information
        bedrooms = details.get('Số phòng') or details.get('rooms')
        ...
        bathrooms = details.get('Số toilet') or details.get('Số phòng tắm') or details.get('bathrooms') or details.get('toilets')
        ...
        floors = details.get('Số tầng', listing.get('Số tầng') or details.get('floors'))
        ...
        raw_furniture = details.get('Nội thất',{}) or details.get('furniture') or {}
        furniture = self._normalize_furniture(raw_furniture)
        
        # Extract width and length - handle both direct values and from description
        width = details.get('Chiều ngang',{}) or details.get('Chiều rộng') or details.get('entrance_width') or {}
        width_value = self._extract_numeric_value(width) if width else ''
        
        length = details.get('Chiều dài',{}) or details.get('facade_width') or details.get('depth') or {}
        ...
        details_data = {
            ...
            'direction': details.get('Hướng nhà', 'Không có thông tin'),
            'balcony_direction': details.get('balcony_direction', 'Không có thông tin'),
            'road_width': details.get('Đường vào', 'Không có thông tin'),
            'road_width_value': self._extract_numeric_value(details.get('Đường vào', '')),
            'house_front': details.get('Mặt tiền', 'Không có thông tin'),
            'house_front_value': self._extract_numeric_value(details.get('Mặt tiền', '')),
            'width': width,
    ...
    def _transform_raw_contact_info(self, listing: Dict[str, Any], listing_id: str) -> Dict[str, Any]:
        ...
        contact = listing.get('contact', {}) or listing 
        contact_name = self._clean_text(listing.get('Tên liên lạc', '') or listing.get('contact_name', '')) or {}
        contact_phone = listing.get('Di động', '') or listing.get('phone', '') or listing.get('mobile', '')
        contact_address = self._clean_text(listing.get('Địa chỉ', '') or listing.get('address', '')) or {}
